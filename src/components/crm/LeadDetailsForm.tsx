...
const INTEREST_TYPES = ["Residencias", "Penthouses", "Áreas de confort", "Corporativos", "Comercio"];
const PAYMENT_METHODS = ["Contado", "Crédito", "Ambos"];
const PURPOSES = ["Vivienda", "Inversión"];
  ...
  if (!editing) {
    return (
      <div>
        ...
        <dl className="divide-y divide-[var(--crm-line)]">
          <Prop label="Tipo de interés" value={q.interestType ?? null} />
          <Prop label="Presupuesto" value={q.budget ?? null} />
          <Prop label="Forma de pago" value={q.paymentMethod ?? null} />
          <Prop label="Uso" value={q.purpose ?? null} />
          <Prop label="Plazo de compra" value={q.timeline ?? null} />
        </dl>
      </div>
    );
  }
  ...
  return (
    <form action={async (fd) => { await action(fd); setEditing(false); }} className="flex flex-col">
      ...
      <fieldset className="space-y-3">
        ...
        <div>
          <label className={labelCls} htmlFor="ld-interestType">Tipo de interés</label>
          <select id="ld-interestType" name="interestType" defaultValue={q.interestType ?? ""} className={inputCls}>
            <option value="">Sin definir</option>
            {INTEREST_TYPES.map((n) => (
              <option key={n} value={n}>{n}</option>
            ))}
          </select>
        </div>
        <div>
          <label className={labelCls} htmlFor="ld-budget">Presupuesto</label>
          <input id="ld-budget" name="budget" defaultValue={q.budget ?? ""} className={inputCls} />
        </div>
        <div>
          <label className={labelCls} htmlFor="ld-paymentMethod">Forma de pago</label>
          <select id="ld-paymentMethod" name="paymentMethod" defaultValue={q.paymentMethod ?? ""} className={inputCls}>
            <option value="">Sin definir</option>
            {PAYMENT_METHODS.map((n) => (
              <option key={n} value={n}>{n}</option>
            ))}
          </select>
        </div>
        <div>
          <label className={labelCls} htmlFor="ld-purpose">Uso</label>
          <select id="ld-purpose" name="purpose" defaultValue={q.purpose ?? ""} className={inputCls}>
            <option value="">Sin definir</option>
            {PURPOSES.map((n) => (
              <option key={n} value={n}>{n}</option>
            ))}
        ...
        <div>
          <label className={labelCls} htmlFor="ld-timeline">Plazo de compra</label>
          <input id="ld-timeline" name="timeline" defaultValue={q.timeline ?? ""} className={inputCls} />
        </div>
      </fieldset>
